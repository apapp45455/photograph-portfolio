...
`js/gallery-data.json` → `GalleryDataSource.load()` → `Gallery.render()` → DOM grid → click → `Lightbox` opens, and `ExifMetadataReader` fetches the `exif-js` CDN script *at that moment* before reading EXIF off the original image. `Gallery` takes a `select` function that narrows/reorders the manifest for the page it is on: the home page passes `selectUngrouped`, a series page passes `selectSeriesPhotos(series)`. Whatever `select` returns is both what renders and what the lightbox pages through.
...
- `exif.js` — `ExifMetadataReader` (reads EXIF from the original image; disabled on `file:` protocol), `MetadataRenderer` (metadata grid in lightbox). **There is deliberately no `<script src="…exif-js">` tag on either page**: a deferred classic script shares one ordered list with the module entry point, so `DOMContentLoaded` — and with it `App.start()` and the JS-built series cover that is the LCP element — waited on cdn.jsdelivr.net. `defer` does not fix that; only not having the tag does. The module fetches the script itself on the first lightbox open, retrying on any load that yields no reader (a captive portal answering `200 text/html` fires `load`, not `error`)
...
| `gallery` | `scripts/check-gallery.js --deep` (files, dimensions, orphans, the series rules the generator does not enforce, plus two couplings that live outside the manifest: `checkHomePreload` ties `index.html`'s cover preload to `SeriesCardRenderer.createCover`, and `checkGridTiers` ties `CONFIG.GRID_TIERS` to the generator's `IMAGE_SIZES` — in both directions, so the list can neither be renamed out of step nor grow `large` back) **plus `npm run check:generated`, which re-runs `generate-gallery.js` and diffs the result** (the same script `npm test` runs, so the local suite is not green on the one change the gate exists to catch) — that diff, not a hand-written comparison, is what catches a `series.json` edit committed without a rebuild |
...
**Known cost, not yet fixed: the lightbox downloads the full original to read EXIF.** `ExifMetadataReader.read()` is handed `item.original` — the compressed but full-resolution file in `images/` — purely to parse its header. The lightbox never displays it; `LightboxView.showItem` shows `-large.jpg`. Across the 18 photos that is 30,884 KB fetched against 6,911 KB shown (4.5x), the worst single frame is 3,563 KB fetched for 1,152 KB shown, and `updateImage()` repeats it on every next/prev — paging the 5-photo japan series costs 2,489 KB visible plus 11,370 KB invisible. It does not affect LCP (it happens on interaction, not load), which is why it survived the LCP work. The fix is extracting EXIF at build time with `exifr` — see the note in `generate-gallery.js` — which also deletes `js/exif.js`'s CDN loader and the exif-js dependency entirely. Do not "optimise" this by pointing `read()` at a derivative: `generate-gallery.js` strips EXIF, so the derivatives have no header to parse.
