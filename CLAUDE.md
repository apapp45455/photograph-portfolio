...
- `utils.js` — shared helpers (`formatPhotoTitle`, `createElement`, srcset builders) + JSDoc typedefs. `getVersionSrcset` percent-encodes spaces and commas in every candidate URL: those two characters are srcset's own delimiters, so a filename containing one makes the candidate unparseable and the browser **discards the entire `<source>`** — `images/Canon R50特寫.jpg` silently rendered its 400px thumb at every viewport for as long as it had been on the site, with a perfectly correct-looking `srcset` attribute. CJK is deliberately left raw so the manifest and `index.html`'s preload stay readable
